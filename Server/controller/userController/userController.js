...
// Configure your email transport (example with Gmail)
const transporter = nodemailer.createTransport({
  host:'mail.skahmerali.com',
  port:465,
  secure: true,
  auth: {
    user: 'skdigitech@skahmerali.com',
    pass: '3102007KpH0740@$'
  }
});
...
const createUser = async (req, res) => {
  try {
    ...
    const mailOptions = {
      from: 'skdigitech@skahmerali.com',
      to: user.email,
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      if (error) {
        console.log("can't register email: " + error);
        User.findOneAndUpdate({ code: user.code }, { isEmailRegistered: false }).exec();
      } else {
        console.log('Email registered: ' + info.response);
